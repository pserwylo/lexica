...
public abstract class TrieTest {
    ...
    static void assertTrieMatches(String message, Trie trie, String[] words, String[] notWords, boolean strict) {
        String log = message + ": ";
        for (String word : words) {
            Assert.assertTrue(log + word + " should be a word", trie.isWord(word, strict));
        }

        if (notWords != null) {
            for (String notAWord : notWords) {
                Assert.assertFalse(log + notAWord + " should not be a word", trie.isWord(notAWord, strict));
            }
        }
